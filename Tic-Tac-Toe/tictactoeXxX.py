...
def get_board():
    try:
        len = int(input("Please enter the board size:"))
    except ValueError:
...
def print_board(board):
    ...
    for j in range(1,len(board)):
        line += '|{:1}'
    whole_board.append('-'*(len(board)*2-1))
    for i in board:
        line1 = line.format(*i)
        whole_board.append(line1)
        whole_board.append('-'*(len(board)*2-1))
    whole_board  = '\n'.join(whole_board)
    print(whole_board)
...
def player_move(board, player):
    ...
    while True:
        try:
            ...
            print(39 * '=')
            row, column = divmod(move-1, len(board))
            if board[row][column] == '':
...
def main():
    ...
    while True:
        print_board(BOARD)
        BOARD, row, column = player_move(BOARD, PLAYER)
